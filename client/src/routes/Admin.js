...
// Adding locations - Expected behaviour

  // 1 - Adds new location to the DB if unique - TEST
  // 2 - There is a function that checks if a user is a level one user, checks they have ALL locations, if not adds any missing locations to the uuser - TEST
  // 3 - Removing locations in the middle of the year is not permitted as this would remove data



  // Added location and successfuly added location to user without reload or transition
  // Location is not displayed in the tables (awards or tenders) however.
  // next issue is that all the defaults for the tables are generated by a one foof api call so,
    // When a location is added and the user navigates to the awards table there is no default data
  // When a director adds a location, the default data needs to be generated straight away.



  // Generate tender data
    // when and where does this need to happen?
    // whenever the director adds a location the tenders data will also need updating
    // Basically whenever the awards data is being set / updated so should the tenders
